...
import (
	...
	// Removed import of service to fix import cycle
...
func ArticleSentenceTranslateWorker(sentence string) (ArticleSentenceResult, error) {
	util.Log("[ArticleSentenceTranslateWorker] Translating sentence: '"+sentence+"'", util.LogTypeLog)
	result, err := TokenizeSentenceWithAI(sentence)
	if err != nil || !result.Success {
		util.Log("[ArticleSentenceTranslateWorker] Error tokenizing sentence: "+err.Error(), util.LogTypeError)
		return ArticleSentenceResult{}, err
	}
	...
	return ArticleSentenceResult{Sentence: sentence, Words: result.Words}, nil
}

// EnhanceTokenizedWords calls a cheaper AI model to add meanings, JLPT, and frequency to tokenized words
func EnhanceTokenizedWords(result *AITokenizationResult) {
	...
	enhancementMessages := []util.OpenAIChatMessage{
		{Role: "system", Content: "You are a Japanese linguistics expert. Return only valid JSON."},
		{Role: "user", Content: enhancementPrompt},
	}
	enhancementResponse, err := util.CallOpenAIChatCompletion(modelLow, 0.1, enhancementMessages)
	...
	// Update result.Words with enhanced info
	for i := range result.Words {
		if i < len(enhanced) {
			result.Words[i].EnglishMeaning = enhanced[i].EnglishMeaning
			result.Words[i].JLPTLevel = enhanced[i].JLPTLevel
			result.Words[i].FrequencyRank = enhanced[i].FrequencyRank
		}
	}
	util.Log("[ArticleSentenceTranslateWorker] Enhancement complete.", util.LogTypeLog)
}
...
func TokenizeSentenceWithAI(sentence string) (AITokenizationResult, error) {
	...
	messages := []util.OpenAIChatMessage{
		{Role: "system", Content: "You are a Japanese linguistics expert. Return only valid JSON."},
		{Role: "user", Content: basicTokenizationPrompt},
	}
	aiResponse, err := util.CallOpenAIChatCompletion(model, 0.1, messages)
	util.Log("[TokenizeSentenceWithAI] OpenAI raw response: "+aiResponse, util.LogTypeLog)
	if err != nil {
		return AITokenizationResult{
			Success: false,
			Error:   err.Error(),
			Method:  "ai",
		}, err
